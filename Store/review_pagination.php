...
<div class="text-gray-500 text-sm" id="paginationInfo">
    <?php
    $currentCount = min($rowsPerPage, $reviewCount - (($currentPage - 1) * $rowsPerPage));
    echo 'Showing ' . $currentCount . ' of ' . $reviewCount . ' reviews';
    ?>
</div>
<div class="flex justify-center items-center mt-1 gap-1 <?= (!empty($reviewCount)) ? 'flex' : 'hidden' ?>" id="paginationControls">
    <!-- Previous Btn -->
    <?php if ($currentPage > 1): ?>
        <a href="#" onclick="loadReviewPage(<?= $currentPage - 1 ?>); return false;"
    ...
    <?php for ($p = 1; $p <= $totalPages; $p++): ?>
        <a href="#" onclick="loadReviewPage(<?= $p ?>); return false;"
