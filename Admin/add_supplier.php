...
if (isset($_POST['bulkdeletesuppliers'])) {
    $ids = $_POST['supplierids'] ?? [];
    ...
    if (!empty($ids)) {
        ...
        $idsList = implode(',', $ids);
        $deleteQuery = "DELETE FROM suppliertb WHERE SupplierID IN ($idsList)";

        if ($connect->query($deleteQuery)) {
            $response['success'] = true;
            $response['deletedIds'] = $ids;
        } else {
            $response['message'] = 'Failed to delete selected suppliers. Please try again.';
        }
    } else {
        $response['message'] = 'No suppliers selected for deletion.';
    }
...
<body>
    ...
    <div class="flex flex-col md:flex-row md:space-x-3 p-3 ml-0 md:ml-[250px] min-w-[380px]">
        ...
        <div class="w-full bg-white p-2">
            <div class="flex justify-between items-end">
                ...
                <div class="flex gap-2">
                    ...
                    <button id="supplierBulkDeleteBtn"
        ...
        <div id="supplierBulkDeleteModal"
            ...
            <div class="bg-white max-w-lg p-6 rounded-md shadow-md text-center">
                ...
                <p class="text-slate-600 mb-2">
                    You are about to delete <span id="supplierBulkDeleteCount" class="font-semibold">0</span> Suppliers.
                </p>
                <p class="text-sm text-gray-500 mb-4">
                    This action cannot be undone. All selected suppliers will be permanently removed from the system.
                </p>
                <div class="flex justify-end gap-4 select-none">
                    <div id="supplierBulkDeleteCancelBtn"
                        class="px-4 py-2 bg-gray-200 text-black hover:bg-gray-300 rounded-sm cursor-pointer">
                        Cancel
                    </div>
                    <button type="button" id="supplierBulkDeleteConfirmBtn"
                        class="px-4 py-2 bg-red-600 text-white hover:bg-red-700 rounded-sm">
                        Delete
                    </button>
                </div>
