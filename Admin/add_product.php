...
if (isset($_POST['bulkdeleteproducts'])) {
    ...
    if (!empty($ids)) {
        ...
        $idsList = implode(',', $ids);
        $deleteQuery = "DELETE FROM producttb WHERE ProductID IN ($idsList)";

        if ($connect->query($deleteQuery)) {
            $response['success'] = true;
            $response['deletedIds'] = $ids;
        } else {
            $response['message'] = 'Failed to delete selected products. Please try again.';
        }
    } else {
...
<body>
    ...
    <div class="flex flex-col md:flex-row md:space-x-3 p-3 ml-0 md:ml-[250px] min-w-[380px]">
        ...
        <!-- Products Bulk Delete Confirm Modal -->
        <div id="productBulkDeleteModal"
            ...
            <div class="bg-white max-w-lg p-6 rounded-md shadow-md text-center">
                ...
                <p class="text-slate-600 mb-2">
                    You are about to delete <span id="bulkDeleteCount" class="font-semibold">0</span> Products.
                </p>
                <p class="text-sm text-gray-500 mb-4">
                    This action cannot be undone. All selected products will be permanently removed from the system.
                </p>
                <div class="flex justify-end gap-4 select-none">
                    <div id="bulkDeleteCancelBtn"
                        class="px-4 py-2 bg-gray-200 text-black hover:bg-gray-300 rounded-sm cursor-pointer">
                        Cancel
                    </div>
                    <button type="button" id="bulkDeleteConfirmBtn"
                        class="px-4 py-2 bg-red-600 text-white hover:bg-red-700 rounded-sm">
                        Delete
                    </button>
                </div>
